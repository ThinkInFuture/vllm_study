...
attn_dir = os.path.join(src, 'attention')
...
print(f'Attention module: {len(file_infos)} files, {total_cls} classes, {total_funcs} functions (unique func names)')

# 生成 panorama-attention.html
html_lines = []
html_lines.append('<!DOCTYPE html>')
html_lines.append('<html lang="zh-CN">')
html_lines.append('<head>')
html_lines.append('<meta charset="UTF-8">')
html_lines.append('<title>vLLM Ascend - 注意力模块 API 参考</title>')
...
with open('panorama-attention.html', 'w', encoding='utf-8') as f:
    f.write('\n'.join(html_lines))
print('Generated panorama-attention.html successfully')