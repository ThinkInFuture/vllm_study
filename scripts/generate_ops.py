...
print(f'Ops module: {len(file_infos)} files, {total_cls} classes, {total_funcs} functions (unique func names)')

# 生成 panorama-ops.html
# 策略：每个文件展示前20个函数，其余以 “还有 N 个函数” 结尾
# 同时每个函数名后添加 (功能说明) 占位
...
html_lines.append('<title>vLLM Ascend - 算子模块 API 参考</title>')
...
with open('panorama-ops.html', 'w', encoding='utf-8') as f:
    f.write('\n'.join(html_lines))
print(f'Generated panorama-ops.html successfully ({len(file_infos)} files, {total_funcs} functions)')