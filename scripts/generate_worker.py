...
worker_dir = os.path.join(src, 'worker')
...
print(f'Worker module: {len(file_infos)} files, {total_cls} classes, {total_funcs} functions (unique func names)')

# 生成 panorama-worker.html
# V2架构部分单独列出，V1部分合并展示
html_lines = []
...
html_lines.append('<title>vLLM Ascend - 执行器模块 API 参考</title>')
...
for i, finfo in enumerate(file_infos):
    ...
    # 展示前20个函数
    for fname in funcs[:20]:
        html_lines.append(f'<div class="item"><span class="item-type type-func">func</span><span class="item-name">{fname}</span><span class="item-desc">(功能说明)</span></div>')
    ...
    shown += 1
    if shown % 20 == 0:
        print(f'  Processed {shown}/{len(file_infos)} files...')

html_lines.append('</body>')
html_lines.append('</html>')

with open('panorama-worker.html', 'w', encoding='utf-8') as f:
    f.write('\n'.join(html_lines))
print(f'Generated panorama-worker.html successfully ({len(file_infos)} files, {total_funcs} functions)')